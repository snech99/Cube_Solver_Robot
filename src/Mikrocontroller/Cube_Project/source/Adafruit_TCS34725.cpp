...
void Adafruit_TCS34725::write8(uint8_t reg, uint8_t value) {
  uint8_t buffer_tx[2] = {(uint8_t)(TCS34725_COMMAND_BIT | reg), value};

  LPI2C0_masterTransfer.slaveAddress = I2C_ADDR_SENSOR;
...
uint8_t Adafruit_TCS34725::read8(uint8_t reg) {
  uint8_t buffer_tx[1] = {(uint8_t)(TCS34725_COMMAND_BIT | reg)};
  uint8_t buffer_rx[1] = {};

  LPI2C0_masterTransfer.slaveAddress = I2C_ADDR_SENSOR;
  LPI2C0_masterTransfer.flags = kLPI2C_TransferDefaultFlag;
  LPI2C0_masterTransfer.direction = kLPI2C_Write;
  LPI2C0_masterTransfer.dataSize = 1;
  LPI2C0_masterTransfer.data = buffer_tx;
  LPI2C_MasterTransferBlocking(LPI2C0, &LPI2C0_masterTransfer);

  LPI2C0_masterTransfer.direction = kLPI2C_Read;
  LPI2C0_masterTransfer.dataSize = 1;
...
uint16_t Adafruit_TCS34725::read16(uint8_t reg) {
  ...
  return (uint16_t(buffer_rx[1]) << 8) | (uint16_t(buffer_rx[0]) & 0xFF);
}
...
void Adafruit_TCS34725::clearInterrupt() {
  uint8_t buffer_tx[1] = {TCS34725_COMMAND_BIT | 0x66};
